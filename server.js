...
client.on("guildMemberRemove", async member => {
  ...
  if (!d) {
    const aa = new Discord.RichEmbed()
      .setColor("RED")
      .setDescription(
        `  <@${member.user.id}> **Adlı şahıs aramızdan ayrıldı.\nŞahsı davet eden:**  Bulunamadı!`
      )
      .setFooter(client.user.username, client.user.avatarURL);
...
.setColor("GREEN")
      .setDescription(
        `  <@${member.user.id}>**Adlı şahıs sunucuya katıldı.\nŞahsı davet eden:** <@${davetçi.id}>**Toplam \`\`${sayı2}\`\` daveti oldu!** ` 
      )
      .setFooter(client.user.username, client.user.avatarURL);
    client.channels.get(kanal).send(aa);
    if (!veri) return;
...
client.on("guildMemberAdd", async member => {
  ...
  if (!msj) {
    const embed = new Discord.RichEmbed()
      .setColor("BLACK")
      .setDescription(
        ` :loudspeaker: <@${member.user.id}>adlı şahsa rolü verildi! :inbox_tray:`
      )
      .setFooter(client.user.username, client.user.avatarURL);
    client.channels.get(kanal).send(embed);
    return;
